...
Notation i := (separated _ _ _ MPLSVect.aut).
Notation "⟦ x ⟧" := (interp_crel i x).
Notation "⦇ x ⦈" := (interp_conf_rel (a:=MPLSVect.aut) x).
...
Lemma no_state:
  forall R S,
    (forall q1 q2 (_ : interp_crel (separated _ _ _ MPLSVect.aut) R q1 q2),
    ...
    (forall st1 (buf1: n_tuple bool S.(cr_st).(cs_st1).(st_buf_len)) st2 (buf2: n_tuple bool S.(cr_st).(cs_st2).(st_buf_len)),
        ...
        interp_crel (separated _ _ _ MPLSVect.aut) R q1 q2 ->
  ...
  | |- exists (x: P4A.store ?H), @?P x =>
    cut (exists y0 y1 y2 y3 y4 y5,
                P ([(inl MPLSPlain.HdrMPLS, P4A.VBits y0);
                    ...
                    (inl MPLSUnroll.HdrMPLS, P4A.VBits y3);
                    (inl MPLSUnroll.HdrMPLS1, P4A.VBits y4);
                    (inl MPLSUnroll.HdrUDP, P4A.VBits y5)]));
    [ intros [x0 [x1 [x2 [x3 [x4 [x5 ?]]]]]];
      exists ([(inl MPLSPlain.HdrMPLS, P4A.VBits x0);
              ...
              (inl MPLSUnroll.HdrMPLS, P4A.VBits x3);
              (inl MPLSUnroll.HdrMPLS1, P4A.VBits x4);
              (inl MPLSUnroll.HdrUDP, P4A.VBits x5)]);
      eauto
...
Proof.
  idtac "running MPLS plain <-> unrolled small bisimulation".
