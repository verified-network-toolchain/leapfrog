...
Require Import Poulet4.P4automata.Examples.EthSmall.
From Hammer Require Import Tactics.

Notation i := (separated _ _ _ RefComb.aut).
Notation "⟦ x ⟧" := (interp_crel i x).
Notation "⦇ x ⦈" := (interp_conf_rel (a:=RefComb.aut) x).
...
Lemma no_state:
  forall R S,
    (forall q1 q2 (_ : interp_crel (separated _ _ _ RefComb.aut) R q1 q2),
    ...
    (forall st1 (buf1: n_tuple bool S.(cr_st).(cs_st1).(st_buf_len)) st2 (buf2: n_tuple bool S.(cr_st).(cs_st2).(st_buf_len)),
        ...
        interp_crel (separated _ _ _ RefComb.aut) R q1 q2 ->
...
Ltac break_store := 
  ...
  | |- exists (x: P4A.store ?H), @?P x =>
    cut (exists y0 y1 y2 y3 y4,
                P ([(inl Reference.HPref, P4A.VBits y0);
                    (inl Reference.HDest, P4A.VBits y1)
                    (inl Reference.HSrc, P4A.VBits y2);
                    (inl Reference.HProto, P4A.VBits y3);
                    (inl Combined.HdrVar, P4A.VBits y4)]));
    [ intros [x0 [x1 [x2 [x3 [x4 ?]]]]];
      exists ([(inl Reference.HPref, P4A.VBits x0);
      (inl Reference.HDest, P4A.VBits x1)
      (inl Reference.HSrc, P4A.VBits x2);
      (inl Reference.HProto, P4A.VBits x3);
      (inl Combined.HdrVar, P4A.VBits x4)]);
      eauto
...
Lemma prebisim:
  pre_bisimulation RefComb.aut
                   (WPSymLeap.wp (H:=_))
                   (separated _ _ _ RefComb.aut)
                   nil
                   (mk_init 10 RefComb.aut Reference.SPref Combined.Parse)
                   (inl (inl Reference.SPref), [], [])
                   (inl (inr Combined.Parse), [], []).
Proof.
  idtac "running ethernet reference <-> combined small bisimulation".
  set (rel0 := mk_init 10 RefComb.aut Reference.SPref Combined.Parse).
  cbv in rel0.
  subst rel0.
